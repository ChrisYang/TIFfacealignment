...
// ----------------------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------------------
        ...
        !*/
        {
            ...
            std::cout << "pixel size: " <<  feature_pixel_values.size() << std::endl;
...
// ----------------------------------------------------------------------------------------
    ...
    private:
        ...
        {
            if (_verbose)
            std::cout << "set random initialisations" << std::endl;
            samples.clear();
            ...
            for (unsigned long i = 0; i < samples.size(); ++i)
            {
                ...
                {
                    if ((i%get_oversampling_amount()) == 1){
                    //add noise to target shape in order to train a tracking model 
                    ...
                    samples[i].current_shape = (1 - alpha) * samples[i].target_shape + alpha * samples[rand_idx].target_shape;
                    // std::cout << "Current Shape: " <<  samples[i].current_shape << std::endl;
                    // std::cout << "Target Shape :"  << samples[i].target_shape << std::endl;
